...
void Activity::Init(void) {
    ...
    imu_sub_ptr_ = std::make_shared<IMUSubscriber>(
        private_nh_, imu_config_.topic_name, 1000000);
    ...
    odom_ground_truth_sub_ptr = std::make_shared<OdomSubscriber>(
        private_nh_, odom_config_.topic_name.ground_truth, 1000000);
...
Activity::GetUnbiasedAngularVel(const Eigen::Vector3d &angular_vel) {
 ...
 */
inline Eigen::Vector3d
Activity::GetUnbiasedLinearAcc(const Eigen::Vector3d &linear_acc,
 ...
 */
bool Activity::GetAngularDelta(const size_t index_curr, const size_t index_prev,
                               Eigen::Vector3d &angular_delta) {
    //
    ...
    const IMUData &imu_data_curr = imu_data_buff_.at(index_curr); // 1
    const IMUData &imu_data_prev = imu_data_buff_.at(index_prev); // 0
 ...
 * @return true if success false otherwise
 */
bool Activity::GetVelocityDelta(const size_t index_curr,
 ...
 * @return void
 */
void Activity::UpdateOrientation(const Eigen::Vector3d &angular_delta,
 ...
 * @return void
 */
void Activity::UpdatePosition(const double &delta_t,
